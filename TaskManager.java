package ru.yandex.practicum.Финальные_спринты.Четвертый_Спринт;
...
public class TaskManager {
    ...
    public Epic updateEpic(Epic epic) {
        Integer epicID = epic.getId(); // Получаем идентификатор эпика
        ...
        if (!oldEpicSubtaskList.isEmpty()) {
            for (Subtask subtask : oldEpicSubtaskList) {
                subtasks.remove(subtask.getId()); // Удаляем подзадачи из мапы подзадач
            }
        }
        epics.replace(epicID, epic); // Обновляем эпик в мапе
